...
class Navigator {
    ...
    ;
    get platform() {
        if (window.layabox.devinfo.os == "OpenHarmony")
            return "ohos";
        return window.layabox.devinfo.os;
    }
    get userAgent() {
...
class XMLHttpRequest extends EventTarget {
    ...
    getResponseHeader(name) {
        if (this._readyState < 2 || !this._head) {
            return null;
        }
        var lowerName = name.toLowerCase();
        var values = [];
        var headers = this._head.split(/\r?\n/);
        for (var i = 0; i < headers.length; i++) {
            ...
            if (headerName.toLowerCase() === lowerName) {
                values.push(header.substring(colonIndex + 1).trim());
            }
        }
        if (values.length === 0) {
            return null;
    ...
    send(body) {
        ...
        var onPostLoad = function (buf, strbuf) {
            var _t = this._t;
            if (_t._method == 'HEAD') {
                _t._response = _t._responseText = "";
            }
            else if (_t.responseType == 'arraybuffer') {
                _t._response = buf;
            ...
            _t._readyState = 4;
            _t._status = 200;
            _t._head = _t.xhr.get_responseHeaders() || "";
...
class LayaGLContext {
    constructor(contextType, nativeObj) {
        this.width = 0;
        this.height = 0;
        this.canvas = null;
        ...
        this._frameAndSyncCountBuffer = new Int32Array(2);
        this._frameAndSyncCountBuffer["conchRef"] = conch.createArrayBufferRef(this._frameAndSyncCountBuffer, LayaGLContext.ARRAY_BUFFER_TYPE_DATA, false, LayaGLContext.ARRAY_BUFFER_REF_REFERENCE);
        this._frameAndSyncCountBuffer["_ptrID"] = this._frameAndSyncCountBuffer["conchRef"].id;
        this._frameAndSyncCountBuffer[0] = 1;
        this._frameAndSyncCountBuffer[1] = 0;
        this._nativeObj = nativeObj;
        contextType = contextType.toLowerCase();
        if (contextType.indexOf("layagl") >= 0) {
            this._nativeObj.setFrameAndSyncCountArrayBufferID(this._frameAndSyncCountBuffer["_ptrID"]);
    ...
    getFrameCount() {
        return this._frameAndSyncCountBuffer[0];
    }
...
class CSSStyleDeclaration {
    ...
    get fontFamily() {
        return this._htmlEle._nativeObj.fontFamily;
    }
    set fontFamily(value) {
        this._htmlEle._nativeObj.fontFamily = value;
    }
    get color() {
        return this._htmlEle._nativeObj.color;
    }
    set color(value) {
        this._htmlEle._nativeObj.color = value;
    }
    get fontSize() {
        return this._htmlEle._nativeObj.fontSize;
    }
    set fontSize(value) {
        this._htmlEle._nativeObj.fontSize = value;
    }
    get padding() {
        return this._htmlEle._nativeObj.padding;
    }
    set padding(value) {
        this._htmlEle._nativeObj.padding = value;
    }
    get textAlign() {
        return this._htmlEle._nativeObj.textAlign;
    }
    set textAlign(value) {
        this._htmlEle._nativeObj.textAlign = value;
    }
    get fontWeight() {
        return this._htmlEle._nativeObj.fontWeight;
    }
    set fontWeight(value) {
        this._htmlEle._nativeObj.fontWeight = value;
    }
    get fontStyle() {
        return this._htmlEle._nativeObj.fontStyle;
    }
    set fontStyle(value) {
        this._htmlEle._nativeObj.fontStyle = value;
    }
    get whiteSpace() {
        return this._htmlEle._nativeObj.whiteSpace;
    }
    set whiteSpace(value) {
        this._htmlEle._nativeObj.whiteSpace = value;
    }
    get lineHeight() {
        return this._htmlEle._nativeObj.lineHeight;
    }
    set lineHeight(value) {
        this._htmlEle._nativeObj.lineHeight = value;
    }
}
...
class HTMLCanvasElement extends HTMLElement {
    constructor() {
        ...
        this._isWebGL = window.conchConfig.getGraphicsAPI() == 2;
        this.tagName = "CANVAS";
        this._nativeObj = new window.conch.__createCanvas();
    }
    ...
    getContext(contextType) {
        if (contextType.indexOf("webgl") >= 0) {
            if (this._isWebGL) {
                if (!this._context) {
                    this._context = new LayaGLContext(contextType, this._nativeObj.getContext(contextType));
                }
                return this._context;
            }
            else {
                return this._nativeObj.getContext(contextType);
            }
        }
        else if (contextType.indexOf("2d") >= 0) {
            return this._nativeObj.getContext(contextType);
        }
        else {
            return null;
    ...
    toDataURL(type, quality) {
        return this._nativeObj.toDataURL(type, quality);
    }
    set width(w) {
        this._nativeObj.width = w;
    }
    get width() {
        return this._nativeObj.width;
    }
    set height(h) {
        this._nativeObj.height = h;
    }
    get height() {
...
class HTMLInputElement extends HTMLElement {
    ...
    get readOnly() {
        return this._nativeObj.readOnly;
    }
    set readOnly(value) {
        this._nativeObj.readOnly = value;
    }
    get placeholder() {
        return this._nativeObj.placeholder;
    }
    set placeholder(value) {
        this._nativeObj.placeholder = value;
    }
    get maxLength() {
        return this._nativeObj.maxLength;
    }
    set maxLength(value) {
        this._nativeObj.maxLength = value;
    }
    get type() {
