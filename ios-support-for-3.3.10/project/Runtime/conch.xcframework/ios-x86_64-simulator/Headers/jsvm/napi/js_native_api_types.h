...
typedef enum {
  napi_module_uninstantiated,
  napi_module_instantiating,
  napi_module_instantiated,
  napi_module_evaluating,
  napi_module_evaluated,
  napi_module_errored
} napi_module_status;

// Callback for resolving module import specifiers during instantiation.
// Should return the resolved napi_module for the given specifier, or NULL on
// failure.
typedef napi_module(NAPI_CDECL* napi_module_resolve_callback)(
    napi_env env,
    const char* specifier,
    napi_module referrer,
    void* data);

// Callback for dynamic import() expressions.
// Should resolve/reject the returned promise with the module namespace.
typedef napi_value(NAPI_CDECL* napi_import_module_dynamically_callback)(
    napi_env env,
    const char* specifier,
    napi_value resource_name,
    void* data);

// Callback for initializing import.meta object.
typedef void(NAPI_CDECL* napi_import_meta_callback)(
    napi_env env,
    napi_module module,
    napi_value meta,
    void* data);

// Callback for synthetic module evaluation.
typedef napi_status(NAPI_CDECL* napi_synthetic_module_evaluation_callback)(
    napi_env env,
    napi_module module,
    void* data);
