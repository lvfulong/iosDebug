...
// resource_name is used as the module's file name for error messages and
// debugging. Pass NULL to use a default name.
NAPI_EXTERN napi_status NAPI_CDECL
napi_compile_module(napi_env env,
                    napi_value source,
                    const char* resource_name,
                    napi_module* result);

// Instantiate a module and resolve its dependencies.
// The resolve_callback is called for each `import` specifier in the module.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_instantiate(napi_env env,
                        napi_module module,
                        napi_module_resolve_callback resolve_cb,
                        void* data);

// Evaluate a previously instantiated module.
// Returns a Promise value in *result.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_evaluate(napi_env env,
                     napi_module module,
                     napi_value* result);

// Get the namespace object of a module (the object with all its exports).
// The module must be at least instantiated.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_get_namespace(napi_env env,
                          napi_module module,
                          napi_value* result);

// Get the current status of a module.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_get_status(napi_env env,
                       napi_module module,
                       napi_module_status* result);

// Get the exception from an errored module.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_get_exception(napi_env env,
                          napi_module module,
                          napi_value* result);

// Get the number of import requests (static imports) in a module.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_get_requests_count(napi_env env,
                               napi_module module,
                               size_t* result);

// Get the specifier string for the i-th import request.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_get_request_specifier(napi_env env,
                                  napi_module module,
                                  size_t index,
                                  napi_value* result);

// Set a callback that will be called for dynamic import() expressions.
NAPI_EXTERN napi_status NAPI_CDECL
napi_set_import_module_dynamically_callback(
    napi_env env,
    napi_import_module_dynamically_callback callback,
    void* data);

// Set a callback that will be called to initialize import.meta objects.
NAPI_EXTERN napi_status NAPI_CDECL
napi_set_import_meta_callback(napi_env env,
                              napi_import_meta_callback callback,
                              void* data);

// Create a synthetic module with the given export names.
// The evaluation_cb will be called when the module is evaluated;
// use napi_synthetic_module_set_export inside it to set export values.
NAPI_EXTERN napi_status NAPI_CDECL
napi_create_synthetic_module(napi_env env,
                             const char* module_name,
                             size_t export_count,
                             const napi_value* export_names,
                             napi_synthetic_module_evaluation_callback eval_cb,
                             void* data,
                             napi_module* result);

// Set an export value on a synthetic module.
NAPI_EXTERN napi_status NAPI_CDECL
napi_synthetic_module_set_export(napi_env env,
                                 napi_module module,
                                 napi_value export_name,
                                 napi_value export_value);

// Delete/release a module handle.
NAPI_EXTERN napi_status NAPI_CDECL
napi_module_delete(napi_env env, napi_module module);
